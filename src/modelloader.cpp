...
{
    // Reset nodeIndex for every load
    nodeIndex = 0;
...
QSharedPointer<MaterialInfo> ModelLoader::processMaterial(aiMaterial *material)
{
    ...
    else
    {
        ...
        mater->Ambient *= .2f;
        if( mater->Shininess == 0.0) mater->Shininess = 16;
        ...
        if (material->GetTextureCount(aiTextureType_SPECULAR) > 0) {
            qDebug() << "Specular Texture(s) Found:" << material->GetTextureCount(aiTextureType_SPECULAR)
                     << "for Material:" << mater->Name;
            aiString texPath;

            if (material->GetTexture(aiTextureType_SPECULAR, 0, &texPath, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
                QString texturePath = texPath.data;
                mater->isTexture = true;
                mater->specTexName = texturePath;
        ...
        if (material->GetTextureCount(aiTextureType_HEIGHT) > 0) {
            qDebug() << "Normal Texture(s) Found:" << material->GetTextureCount(aiTextureType_HEIGHT)
                     << "for Material:" << mater->Name;
            aiString texPath;

            if (material->GetTexture(aiTextureType_HEIGHT, 0, &texPath, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
                QString texturePath = texPath.data;
                mater->isTexture = true;
                mater->normTexName = texturePath;
...
QSharedPointer<modelMesh> ModelLoader::processMesh(aiMesh *mesh)
{
    QSharedPointer<modelMesh> newMesh(new modelMesh);
...
//    else
//    {
//        // m_vertices grow as more meshes are read
//        // Generate UV if needed, just U and V, no W
//        qDebug() << "Generating UV with Assimp";
//        if (m_textureUV.isEmpty()) m_textureUV.resize(1);
//        QVector<float> temp;
//        temp = m_textureUV.front();
//        temp.resize((m_vertices.size()/3)*2);

//        int nind = vertindexoffset * 2;

//        for(uint ii=0; ii<mesh->mNumVertices; ++ii)
//        {
//            mesh->
//            aiVector3D *vec = mesh->mTextureCoords[ii];
//            qDebug() << vec->x;
//            temp[nind] = vec->x;
//            temp[nind+1] = vec->y;
//            nind += 2;
...
//    static int nodeIndex = 0;
    ...
    {
        QSharedPointer<modelMesh> mesh = m_meshes[node->mMeshes[imesh]];
